...
impl JwksClient {
    ...
    async fn get_decoding_key(&self, kid: &str) -> Result<DecodingKey, AppError> {
        ...
        info!(
            "JWKS Client: Key with kid '{}' not in cache. Fetching JWKS.",
            kid
        );
        let jwks: Jwks = self
            .http_client
        ...
        if let Some(key) = key_map.get(kid) {
            info!(
                "JWKS Client: Successfully retrieved new key with kid '{}'.",
                kid
            );
            Ok(key.clone())
        } else {
...
async fn proxy_handler(
    ...
    let response = backend_request.body(body).send().await.map_err(|e| {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    validation.set_issuer(std::slice::from_ref(&oidc_issuer_external));
